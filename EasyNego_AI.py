                    ...
                    with mui.Paper():
                        with mui.Grid({"container": True, "spacing": 3}):
                            ...
                            with mui.Grid({"item": True, "xs": 6}):
                                ...
                                fig1 = px.bar(df_suppliers, x='Supplier', y='Value (M)', title="Spend by Payment Terms")
                                st.plotly_chart(fig1)
                            with mui.Grid({"item": True, "xs": 6}):
                                st.subheader("Spend development by payment terms (EUR)")
                                fig2 = px.pie(df_suppliers, names='Supplier', values='Value (M)',
                                              title="Spend Development by Payment Terms")
